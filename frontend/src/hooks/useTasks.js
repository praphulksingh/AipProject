...
const TaskForm = ({ onClose, editTask = null }) => {
  ...
  // Initialize all form fields with editTask data if available
  ...
  const [dueDate, setDueDate] = useState(
    editTask && editTask.dueDate ? new Date(editTask.dueDate).toISOString().split('T')[0] : ''
  );
  ...
  useEffect(() => {
    if (editTask) {
      ...
      setRecurringPattern(editTask.recurring || 'Daily');
    }
  }, [editTask]);
